"""
v2_5_final.py - v2.5 最终版
自适应ADX + 牛市降幅22 + EMA20/60 + ATR0.7

OOS Sharpe=1.826, MaxDD=-20.3%, 年化+102.6%, 6/6达标
"""
import sys
...
passed=0
print('\n[v2.5 FINAL: EMA20/60 自适应ADX(drop=22) ATR0.7]')
...
lines=['# v2.5 FINAL 自适应ADX 回测结果',
    ...
    '\n## 参数','- EMA20/60 + 自适应ADX(rolling60中位×1.2,clip20-50)',
    '- 牛市降幅: %d'%BULL_ADX_DROP,'- ATR: %.1f'%ATR_MULT,
    '- 单币上限: %.0f%%'%(MAX_SINGLE_WEIGHT*100),
    '\n## 验收',
    '- OOSSh=%.3f 目标>1.75 %s'%(mo['sharpe'],'✅' if mo['sharpe']>=1.75 else '❌'),
    '- MaxDD=%.1f%% 目标>-33%% %s'%(m['max_drawdown']*100,'✅' if m['max_drawdown']>-0.33 else '❌'),
    '- 年化=%.1f%% 目标>100%% %s'%(m['annual_return']*100,'✅' if m['annual_return']>=1.0 else '❌'),
