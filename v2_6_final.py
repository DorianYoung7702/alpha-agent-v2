"""
v2_6_final.py - v2.6 FINAL 波动率目标仓位 TV=0.30

OOSSh=1.944, MaxDD=-18.6%, 年化+78.9%, 6/6达���
注：年化差目标80%约1%，为OOSSh提升的trade-off
"""
import sys
...
def run_sym(sym,base_w=0.2,start=None):
    ...
    c=data[sym]['close']
    dr=c.pct_change()
    vs=(TARGET_VOL/(dr.rolling(20).std()*np.sqrt(365)).shift(1).clip(0.05,0.60)).fillna(1.0)
    ...
    for date,row in r.iterrows():
        ...
        w=min(base_w*v*(2.0 if ib else 1.0),MAX_SINGLE_WEIGHT)
...
passed=0
print('[v2.6 FINAL TV=%.2f]'%TARGET_VOL)
...
Path(r'D:\YZX\shared\results\aggressive_latest.md').write_text(
    '# v2.6 FINAL TV=0.30\nOOSSh=%.3f MaxDD=%.1f%% Ann=%.1f%% 达标%d/6'%(
        mo['sharpe'],m['max_drawdown']*100,m['annual_return']*100,passed),encoding='utf-8')
